# Ontology Dashboard Operations API 명세서

- 상태: 멘토링 합의 기반 Operations 구현 계약
...
이 문서는 [`operations-scope-and-screen-specification.md`](./operations-scope-and-screen-specification.md)에 정의된 네 개 Operations 화면이 백엔드와 어떤 계약으로 통신해야 하는지를 고정한다.
...
Operations 구현자는 이 문서를 기준으로 화면마다 임의의 API 조합이나 임의 필드 해석을 추가하지 않는다.
...
다음 API 군은 기존 구현을 유지하되 네 개 Operations 화면의 필수 호출에 포함하지 않는다.
...
### 4.1 Operations 사용자 그룹

| Operations 사용자 | 실제 역할 코드 | 기본 화면 |
|---|---|---|
...
Operations에서는 별도 aggregate endpoint 없이 Event 목록에서 계산한다.
...
다음 endpoint는 대규모 데이터 확장용이며 Operations P0 필수 구현이 아니다.
...
Operations에서는 `depth=1`을 기본값으로 사용한다.
...
Operations에서는 V1 Report의 Print CSS를 필수로 사용한다. 서버 PDF Export는 시간 허용 시 연결한다.
...
| ID | Gap | 우선순위 | 완료 기준 |
|---|---|---:|---|
| `Operations-API-GAP-01` | Object 서버 필터 부족 | P1 | line, status, risk, criticality, assignee 필터 |
| `Operations-API-GAP-02` | 담당자 배정 영속 API 없음 | P1 | 저장·조회·Activity 반영 |
| `Operations-API-GAP-03` | Activity envelope 비표준 가능성 | P1 | `{items}` 또는 adapter 고정 |
| `Operations-API-GAP-04` | Report 단일 aggregate API 없음 | P2 | 조합 규칙 테스트로 우선 대체 |
| `Operations-API-GAP-05` | Event pagination 없음 | P3 | 대규모 데이터 도입 시 구현 |
...
- Analysis와 Commercial V4 API를 Operations 필수 dependency로 추가하지 않는다.
- 구현 완료 후 gap 표를 갱신한다.
