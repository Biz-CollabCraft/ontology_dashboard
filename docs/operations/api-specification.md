...
- 점검→정비 판단의 canonical command/read API는
  `/api/projects/{project_id}/workspaces/{workspace_id}/maintenance` 아래에 둔다. 점검 요청과
  Operations manual 추천·판단은 `process_manager`, 점검 요청 수락·본인 배정·시작·결과 기록은
  `process_engineer`가 수행한다. 모든 mutation은 `Idempotency-Key`를 요구한다.
