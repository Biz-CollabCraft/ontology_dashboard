...
- 병합 기준 demo `ab63cfe9`: 원격 서버 회귀 **416 passed, 2 skipped, 2 warnings**. 필수 원격 검사 3개 성공, PR #168 병합 완료. 아래 331/68/34는 앞서 실행한 별도 로컬 회귀 묶음으로 합산하지 않는다.
- 최종 재점검: 핵심 계약 28개, replay 24/24, 오류 대응 16/16, 로컬 전체 연결 24/24와 실패 차단. 팀 DB 최신 100대 × 3회 읽기 일치. [최종 안정성 평가](final-demo-final-stability-evaluation.md).
...
- 로컬 화면 구성 회귀는 32개 통과했다. PostgreSQL 연계 1개는 로컬 서버 부재로 건너뛰었고, 이후 PostgreSQL 서비스가 있는 PR CI의 416개 회귀에서 통과했다. 원격 검사 3개가 모두 성공했다.
