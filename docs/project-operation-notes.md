# Project Operation Notes

상태: 팀 공유용 초안
최종 갱신: 2026-09-01
범위: MVP 워크플로우의 AI 검토, 근거 검증, 권한 경계 운영 원칙

이 문서는 팀 레포에서 공유하는 프로젝트 운영 기준이다. 프로젝트가 역할과 권한을 나누고, 검증된
근거만 소비하며, 리뷰에서 발견한 반례를 테스트와 기준으로 고정한 방식을 정리한다.

## 1. 운영 원칙

### 업무 착수 전 역할과 권한 경계를 먼저 확인한다

워크플로우, 요약, 리포트, 조치 흐름을 변경하기 전 프로젝트는 다음 책임을 분리한다.
...
### PR 리뷰는 전체 소비 경로로 확인한다

PR 리뷰는 특정 테스트 하나의 통과 여부만 보지 않는다. AI summary, evidence, report, UI, schema,
action flow를 건드리는 변경은 다음 소비 경로를 따라 확인한다.
...
리뷰 기준은 각 경계에서 의미, 소유권, evidence state가 유지되는지 확인하는 것이다.

### 놓친 반례는 테스트 또는 리뷰 기준으로 되돌린다

리뷰에서 놓친 edge case가 발견되면 다음 리뷰 루프의 입력으로 다룬다. 후속 조치는 다음 중 하나로 남긴다.
...
## 2. 프로젝트에 반영된 공개 가능 결정

현재 팀 문서에서 공유 가능한 아키텍처 방향은 다음과 같다.
...
이 문서는 운영 원칙의 요약본이다. Evidence Package, Product Result Artifact, ViewModel,
...
PR #150은 위 방향을 구현과 리뷰 산출물에 반영한 근거다.
...
구현 근거를 설명할 때는 다음 축으로 나누어 읽는다. PR #150에 병합된 근거와 현재 브랜치의 후속
보강은 커밋/PR 상태를 확인한 뒤 외부 설명에 사용한다.
...
| Domain context 확장 | operation, SOP, inspection, ontology, maintenance history를 domain section으로 나누되 public packet은 하나로 유지한다. | `systems/backend/app/mvp/context_providers.py`, `systems/backend/app/mvp/agent_review_packet.py`, `tests/test_agent_review_packet_golden.py` |

## 5. 외부 설명 문장

프로젝트 운영 방식을 외부에서 설명할 때는 다음 문장을 기준으로 사용한다.

```text
프로젝트 운영 과정에서 업무 착수 전 역할·권한 경계를 먼저 나누고, AI 요약은 검증된
...
현재 문서는 운영 원칙의 요약본으로 유지한다. 상세 결정과 구현 근거는 기존 ADR, plan, PR로 연결한다.
...
- `docs/plans/ai-workflow/` 아래 모든 계획이 구현 완료됐다는 주장
- Agent Review가 작업 요청을 승인하거나 시스템 상태를 변경할 수 있다는 주장
- 누락된 evidence를 AI summary text로 보완할 수 있다는 주장
- local-only verification이 배포된 production behavior와 같다는 주장
