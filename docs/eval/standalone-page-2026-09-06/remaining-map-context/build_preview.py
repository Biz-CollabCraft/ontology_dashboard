...
with TemporaryDirectory(prefix='ontology-sample-preview-') as temp:
    ...
    assert first == {'inserted': len(rows), 'unchanged': 0}
    assert repeat == {'inserted': 0, 'unchanged': len(rows)}
    previews = []
    for asset in assets:
