...
**현재 산출물은 골드셋 초안이며, 검수된 정답셋이 아니다.** 합성 48사례 중 44개에 정답을 제안하고 조건부/생략된 지시 4개는 미정으로 남겼다. 사람 검수 0/48이다. 작성 당시 모델 호출은 0건이었으며, 이후 명시적인 초안 평가로 두 모델 합계 18회 API 요청을 실행했다. 실제 점검 문구는 제공되지 않아 사용하지 않았다.
...
현재 구조 검사: 오류 0. 미검수/미정 release 차단: 48건. 골드셋·모델 비교 관련 검사 테스트: 15 passed. 이 수치는 골드 정답의 정확도나 모델 품질 점수가 아니다.

## 이전 결과 취급

Luna 105/105와 4o-mini 87/105는 이전 합성 사례에서의 탐색적 비교다. 이 새 초안에 대한 결과가 아니다. 최종 비교는 정답 검수·확정 후에만 진행한다. 새 초안의 별도 탐색 비교는 [2026-09-14 결과](draft-comparison-2026-09-14.md)에 기록했다. 결과를 이용한 추가 프롬프트 튜닝은 실행하지 않았다.
