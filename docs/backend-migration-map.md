...
| `live_predictive_maintenance.py` | Mac mini Compose가 `python -m ontology_dashboard.live_predictive_maintenance`로 실행; `gen_data` wall-clock stream과 Runtime Overlay outbox를 읽어 PostgreSQL Dataset Version/Result를 갱신 | `gen_data`(source·Overlay generation) + `app/dataset` + `app/diagnosis` + Infra/composition | `SPLIT` | stream/file adapter와 DB ingest=`infra`/`dataset`; Model Artifact inference·Result/Evidence=`diagnosis`; 장기 실행 daemon wiring=composition | Backend가 source/Overlay 생성 책임을 가져가면 ownership 계약을 위반하지만, 제품 DB ingest와 promoted artifact inference는 Backend consumer 책임이라 한 owner로 이동할 수 없음 | #57/#58/#64에서 canonical ports와 daemon entrypoint가 준비된 뒤 legacy module/Compose command를 교체하고 `gen_data` import·구현 복제를 금지 | `tests/test_live_predictive_maintenance.py`, `tests/test_generator_artifact_publication.py`, `infra/macmini/docker-compose.yml` runtime smoke |
