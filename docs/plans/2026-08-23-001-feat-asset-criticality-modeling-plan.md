...
- Fixture clean-contract commit `40e37b1` is the prerequisite baseline for this plan. It changes `features[].current` to an observation object and replaces `features[].series` with `features[].history { source_ref, points }`; fixture adapter and canonical composer share the same pre-current, timezone-aware, duplicate-instant rejection invariants.
