...
- Resolved for this slice: the first non-SOP domain adapter after operation context is maintenance history. It projects existing Closed-loop, activity, and equipment-history records into read-only `maintenance_history_summary`; inventory, work schedule, and MES actuals remain future adapters until they have current source contracts.
