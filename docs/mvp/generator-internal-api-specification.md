...
현재 Generator 데몬에 실제로 구현되어 동작하는 엔드포인트는 다음과 같습니다.
...
| POST | `/feature` | Observation + Failure + Plan/Mapping을 소비하여 Feature/Label 및 Feature Bundle 발행 (신규 3단계) | Implemented — local file adapter, validation in progress |
