...
이 폴더는 `Biz-CollabCraft/ontology_dashboard`에서 계속 사용하는 현재 MVP/Product
계약의 canonical namespace다. 2026년 8월 Week 2 당시의 역할 분담, 이관, 비교 분석,
원문 보존 자료는 `history/2026-08-week2/`로 분리한다.

## 현재 계약 읽는 순서

1. [현행 MVP 구현 계약 기준선](./current-mvp-implementation-baseline.md)
2. [MVP 요구사항 명세](./requirements-specification.md)
3. [MVP 기능 명세](./functional-specification.md)
4. [MVP API 명세](./api-specification.md)
5. [MVP 공통 스키마 정의](./schema-definition.md)
6. [MVP 리포트 명세](./report-specification.md)
...
- 문서 이동 시 `docs/mvp/` current 경로와 history 경로를 구분해 자동리뷰 context가 과거 기록을
  현재 계약처럼 소비하지 않게 한다.
