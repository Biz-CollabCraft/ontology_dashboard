...
| 위험도 시계열 | 없음 | `risk_series[]` (`observed_at`, `failure_probability`, `status_grade`, `prediction_id`, `source_kind`) | **불가** — 단일 Event Evidence는 현재값 중심 | Backend Diagnosis가 만들 runtime prediction/result timeline. `gen_data/canonical/model_outputs/prediction_timeline.jsonl`이나 legacy `precomputed_prediction_timeline` 승격 금지 |
...
| 센서 시계열 | 없음 | `features[].series[]` (`observed_at`, `value`, `quality_status`, `source_ref`) | **불가** | versioned Observation contract(canonical/overlay Observation API). gen_data Layer 1/Layer 2/`_log.jsonl` 내부 저장 형태는 Product API의 직접 의존 대상이 아니라 Issue #6 Source Data Producer 수렴 target |
...
| `risk-timeline-present.json` | 위 + Backend Diagnosis runtime prediction/result timeline 연동 | `risk_series`가 `source_kind=runtime_inference`로 채워지고 관련 gap이 사라짐. `equipment_history` gap만 남음 |
...
- `risk_series[].source_ref`가 legacy `precomputed_prediction_timeline`이나 `/timeline`을 직접 가리키지 않는다.
...
- Observation ingestion adapter(§3.2 step 2, `node_id` 파싱/pivot/`status_code` 매핑, §3.1 마지막 문단)는 구현하지 않았다. gen_data Layer 2 `_log.jsonl` 샘플을 Observation API shape로 정규화하는 fixture도 아직 없다.
- Backend Diagnosis의 runtime prediction/result timeline materialization(`risk_series` 공식 소스)은 아직 없다. `risk-timeline-present.json`은 이 소스가 존재한다고 가정한 fixture이며, 실제 producer 구현은 후속 작업이다.
