...
Scope: Asset Detail 후속 작업 중 UI/UX 책임 정리와 역할별 MVP Overview 보강. Agent workflow, Backend risk/criticality/review priority 계산, Closed-loop state machine은 이번 범위에서 제외한다.
...
초기 MVP는 Palantir Ontology식 제품 철학에서 출발했다. 즉 설비, 관측값, 위험 판단, 리포트,
...
둘째, Ontology 철학은 화면에 모든 관계를 드러내는 뜻이 아니다. MVP에서 중요한 것은
...
셋째, 광우 팀원의 Closed-loop 구조가 보여주는 제품 방향은 “리포트를 읽고 끝나는 화면”이 아니라
...
- Closed-loop Product 소비 계약은 핵심 MVP UX를 현장 엔지니어, 생산 운영 의사결정자, 정비 작업자가 이어받는 흐름으로 본다. 따라서 첫 화면도 사용자 역할별 다음 행동을 분리해야 한다.
...
- `systems/frontend/src/features/mvp/overview/MvpOverviewPage.tsx`
- `systems/frontend/src/features/mvp/mvp.css`
- `systems/frontend/src/features/mvp/displayLabels.ts`
- `systems/frontend/src/features/mvp/api/mvpAdapters.ts`
- `systems/frontend/src/features/mvp/api/mvpContracts.ts`
- `systems/frontend/src/features/mvp/report/MvpMapReportAssetDetailView.tsx`
