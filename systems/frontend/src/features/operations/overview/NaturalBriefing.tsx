...
function Briefing(props: Props & { revealed: Set<string> }) {
  ...
  return <section className="natural-briefing" aria-label="AI 자연어 브리핑" aria-busy={busy}>
    ...
    {summary && summary.source_refs.length && !rows.some(row => row.refs.length) ? <EvidenceDisclosure visible refs={summary.source_refs.join("\n")} scope={evidenceScope} label="브리핑 전체 근거"/> : null}
