...
import { displayAssetName, displayAssetShortName, displayEquipmentSensorLabel } from "../displayLabels";
...
export function EngineerFactoryStandalone({
  ...
  const [acknowledgedAlerts, setAcknowledgedAlerts] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(window.localStorage.getItem("engineer.monitoring.acknowledged.v1") ?? "[]") as string[]); }
  ...
  useEffect(() => {
    window.localStorage.setItem("engineer.monitoring.acknowledged.v1", JSON.stringify([...acknowledgedAlerts]));
  }, [acknowledgedAlerts]);
  ...
  const alertKey = (asset: OperationsAsset) => `${asset.assetId}:${asset.eventId ?? asset.observedAt ?? "current"}`;
  const matchesMonitoringCondition = (asset: OperationsAsset) => {
    const isCompressor = asset.assetType.toLowerCase().includes("compress") || asset.assetId.startsWith("CMP-");
    return (zoneFilter === "all" || asset.line === zoneFilter)
      && (equipmentFilter === "all" || (equipmentFilter === "compressor" ? isCompressor : !isCompressor))
      && (statusFilter === "all" || tone(asset.status) === statusFilter)
      && (!unacknowledgedOnly || (tone(asset.status) !== "normal" && !acknowledgedAlerts.has(alertKey(asset))));
  };
  const highlightedAssetCount = model.assets.filter(matchesMonitoringCondition).length;
  ...
  const selectedAcknowledged = selected ? acknowledgedAlerts.has(alertKey(selected)) : false;
  ...
  return (
    <main className="engineer-lite-board">
      ...
      <section className="engineer-monitoring-toolbar" aria-label="설비 감시 조건">
        <div><strong>감시 조건</strong><span>{highlightedAssetCount}대 강조 · 전체 {model.assets.length}대</span></div>
        <label><span>구역</span><select value={zoneFilter} onChange={(event) => setZoneFilter(event.target.value)}><option value="all">전체 구역</option>{[...new Set(model.assets.map((asset) => asset.line))].map((line) => <option key={line} value={line}>{zoneLabel(line)}</option>)}</select></label>
        <label><span>설비</span><select value={equipmentFilter} onChange={(event) => setEquipmentFilter(event.target.value)}><option value="all">전체 설비</option><option value="compressor">압축기</option><option value="cnc">CNC 가공기</option></select></label>
        <label><span>상태</span><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">전체 상태</option><option value="normal">정상</option><option value="attention">주의</option><option value="critical">긴급</option><option value="hold">확인 필요</option></select></label>
        <label className="engineer-unread-filter"><input type="checkbox" checked={unacknowledgedOnly} onChange={(event) => setUnacknowledgedOnly(event.target.checked)} /><span>미확인 알림만</span></label>
        <button type="button" onClick={() => { setZoneFilter("all"); setEquipmentFilter("all"); setStatusFilter("all"); setUnacknowledgedOnly(false); }}>초기화</button>
      </section>
          ...
          <div className="engineer-equipment-scroll">
            {[...groups.entries()].map(([line, assets]) => {
              ...
              return <article key={line} className="engineer-equipment-zone"><div className="engineer-equipment-zone-label"><b>{zoneLabel(line)}</b><small>{assets.length}대</small></div><div className="engineer-equipment-cells">{[...cells.entries()].map(([cell, cellAssets]) => {
                ...
                const assetButton = (asset: OperationsAsset) => <button type="button" key={asset.assetId} aria-label={`${displayAssetShortName(asset)} · ${STATUS_LABEL[asset.status]} · 위험도 ${formatProbability(asset.failureProbability)}`} aria-pressed={asset.assetId === selected?.assetId} style={{"--risk-fill": riskFill(asset)} as CSSProperties} className={`tone-${tone(asset.status)} ${matchesMonitoringCondition(asset) ? "is-highlighted" : "is-muted"} ${asset.assetId === selected?.assetId ? "is-selected" : ""}`} onClick={() => { onSelectAsset(asset.assetId, asset.eventId); setSensorDetailOpen(true); }}><span>{displayAssetShortName(asset)}</span></button>;
      ...
      <section className="engineer-factory-bottom-grid">
        <section className="engineer-factory-card engineer-evidence-summary"><header><strong>선택 설비 근거 요약</strong><span>{selected?.assetId ?? "-"} · {formatTimestamp(selected?.observedAt ?? null)} 관측</span></header>{selected ? <><div className="engineer-evidence-lead"><b>{STATUS_LABEL[selected.status]}</b><strong>{selected.status === "critical" ? "지금 현장 확인과 보전 대응이 필요한 설비입니다." : selected.status === "data_quality_hold" ? "값보다 계측 연결 상태를 먼저 확인해야 합니다." : "현재 상태에 맞춰 관찰과 점검을 이어갑니다."}</strong></div><ol>{selected.topFactors.slice(0, 4).map((factor) => <li key={factor.id}>{displayEquipmentSensorLabel(selected?.assetId, factor.feature, factor.label)} · 기여도 {Math.round(Math.abs(factor.contribution) * 100)}%</li>)}</ol><dl><div><dt>담당자</dt><dd>{selected.assignedEngineer ?? "미배정"}</dd></div><div><dt>예상 정지</dt><dd>{formatMinutes(selected.estimatedDowntimeMinutes)}</dd></div><div><dt>부품</dt><dd>{selected.sparePartAvailable === true ? "확보" : selected.sparePartAvailable === false ? "미확보" : "확인 필요"}</dd></div><div><dt>설비 중요도</dt><dd>{selected.criticality ?? "정보 없음"}</dd></div></dl><p className="engineer-evidence-footer">판단 근거는 현재 선택 설비의 서버 Result와 관측 시각을 기준으로 표시합니다.</p></> : null}</section>
        <section className="engineer-factory-card engineer-live-signals"><header><strong>실시간 상태 신호</strong><span>위험 점수를 밀어올린 신호 상위 {factors.length}건 · {formatTimestamp(selected?.observedAt ?? null)}</span></header><div>{factors.map((factor) => <article key={factor.id}><i><b style={{height: `${Math.max(8, Math.min(100, Math.abs(factor.contribution) * 100))}%`}} /></i><strong>{factor.value?.toLocaleString("ko-KR", {maximumFractionDigits: 2})}<small>{factor.unit ?? ""}</small></strong><span>{displayEquipmentSensorLabel(selected?.assetId, factor.feature, factor.label)}</span><small>현재 관측값</small></article>)}</div><footer className="engineer-signal-note"><span><i />정상 범위 띠</span><span>막대 높이 = 위험 기여도</span><span>서버 관측 품질 기준</span></footer></section>
      </section>

      {sensorDetailOpen && selected ? <div className="engineer-sensor-overlay" onClick={() => { setSensorDetailOpen(false); setExpandedSensor(null); }}><aside className="engineer-sensor-drawer" aria-label={`${displayAssetName(selected)} 상세 현황`} onClick={(event) => event.stopPropagation()}><header><div><strong>{displayAssetName(selected)}</strong><span>{selected.assetId} · 위험 및 정비 요청 현황</span></div><button type="button" onClick={() => { setSensorDetailOpen(false); setExpandedSensor(null); }} aria-label="센서 상세 닫기">×</button></header><div className="engineer-sensor-drawer-body"><section className="engineer-detail-column"><article className="engineer-drawer-risk"><header><div><strong>위험 점수 추세 · 최근 관측</strong><span>{displayAssetName(selected)} · {selected.assetId}</span></div><b className={`tone-${tone(selected.status)}`}>{formatProbability(selected.failureProbability)}</b></header><div className="engineer-drawer-trend"><TrendSvg points={sensorPolyline(selected.riskHistory ?? [])} /><div><span>이전 관측</span><span>현재</span></div></div><footer><span>현재 상태 <b>{STATUS_LABEL[selected.status]}</b></span><span>{selected.riskHistory?.length ?? 0}개 관측 · {formatTimestamp(selected.riskHistory?.[0]?.observedAt)}</span></footer></article><article className="engineer-detail-summary"><header><strong>선택 설비 근거 요약</strong><span>{formatTimestamp(selected.observedAt)} 관측</span></header><p>{selected.status === "critical" ? "즉시 현장 확인과 보전 대응이 필요합니다." : selected.status === "data_quality_hold" ? "센서 연결과 데이터 품질을 먼저 확인해야 합니다." : "현재 상태를 기준으로 관찰과 점검을 이어갑니다."}</p><div className="engineer-detail-actions"><button type="button" className={selectedAcknowledged ? "is-complete" : ""} onClick={() => setAcknowledgedAlerts((current) => { const next = new Set(current); const key = alertKey(selected); if (next.has(key)) next.delete(key); else next.add(key); return next; })}>{selectedAcknowledged ? "알림 확인됨" : "알림 확인"}</button><button type="button" disabled={!maintenanceDirectives.some((item) => item.asset_id === selected.assetId)} title={!maintenanceDirectives.some((item) => item.asset_id === selected.assetId) ? "엔지니어 요청 생성 권한 계약 연결이 필요합니다." : undefined} onClick={() => document.querySelector(".engineer-detail-requests")?.scrollIntoView({ behavior: "smooth", block: "nearest" })}>{maintenanceDirectives.some((item) => item.asset_id === selected.assetId) ? "진행 중 요청 확인" : "보전 점검 요청 연결 필요"}</button></div><dl><div><dt>상태</dt><dd>{STATUS_LABEL[selected.status]}</dd></div><div><dt>담당자</dt><dd>{selected.assignedEngineer ?? "미배정"}</dd></div><div><dt>예상 정지</dt><dd>{formatMinutes(selected.estimatedDowntimeMinutes)}</dd></div><div><dt>부품</dt><dd>{selected.sparePartAvailable === true ? "확보" : selected.sparePartAvailable === false ? "미확보" : "확인 필요"}</dd></div></dl><ol>{selected.topFactors.slice(0, 4).map((factor) => <li key={factor.id}>{displayEquipmentSensorLabel(selected?.assetId, factor.feature, factor.label)} <b>{Math.round(Math.abs(factor.contribution) * 100)}%</b></li>)}</ol></article></section><section className="engineer-drawer-sensors">{selected.sensorHistory?.length ? selected.sensorHistory.map((sensor) => { const latest = sensor.points.at(-1)?.value; return <article key={sensor.feature} role="button" tabIndex={0} onClick={() => setExpandedSensor(sensor.feature)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setExpandedSensor(sensor.feature); }}><header><strong>{displayEquipmentSensorLabel(selected.assetId, sensor.feature, sensor.label)}</strong><span>{latest?.toLocaleString("ko-KR", {maximumFractionDigits: 2})}{sensor.unit ? ` ${sensor.unit}` : ""}</span></header><div className="engineer-drawer-trend"><TrendSvg points={sensorPolyline(sensor.points)} /><div><span>이전 관측</span><span>현재</span></div></div><footer><span>{sensor.points.length}개 관측</span><span>확대해서 보기</span></footer></article>; }) : <p>표시할 센서 관측 이력이 없습니다.</p>}</section><section className="engineer-detail-requests"><header><div><strong>정비 요청 목록</strong><span>먼저 접수된 순서</span></div><span className={`engineer-directive-connection ${maintenanceDirectiveError ? "is-offline" : "is-online"}`}><i />{maintenanceDirectiveError ? "연결 확인 필요" : "목록 연결 정상"}</span></header><div>{!maintenanceDirectiveError && maintenanceDirectives.length ? maintenanceDirectives.map((directive) => <button type="button" key={directive.work_order_id} className={directive.asset_id === selected.assetId ? "is-selected" : ""} onClick={() => { setDirectiveAssetId(directive.asset_id); setExpandedSensor(null); onSelectAsset(directive.asset_id, directive.event_id); }}><span><b>{directive.status === "in_progress" ? "점검 중" : directive.status === "approved" ? "승인됨" : "요청됨"}</b><small>{directive.work_order_id}</small></span><strong>{directive.equipment_id || directive.asset_id}</strong><small>{directive.asset_id}</small><p>{directive.assigned_to ? `담당 ${directive.assigned_to}` : "담당자 배정 대기"}</p></button>) : <p className="engineer-directive-empty">{maintenanceDirectiveError ? "정비 요청 목록을 불러오지 못했습니다" : "현재 정비 요청이 없습니다"}</p>}</div></section></div>{expandedSensor ? (() => { const sensor = selected.sensorHistory?.find((item) => item.feature === expandedSensor); return sensor ? <div className="engineer-sensor-expanded" onClick={() => setExpandedSensor(null)}><article onClick={(event) => event.stopPropagation()}><header><div><strong>{displayEquipmentSensorLabel(selected.assetId, sensor.feature, sensor.label)} 상세 추이</strong><span>{sensor.points.length}개 관측값</span></div><button type="button" onClick={() => setExpandedSensor(null)} aria-label="확대 그래프 닫기">×</button></header><TrendSvg points={sensorPolyline(sensor.points)} /><footer><span>{formatTimestamp(sensor.points[0]?.observedAt)}</span><b>{sensor.points.at(-1)?.value.toLocaleString("ko-KR", {maximumFractionDigits: 2})}{sensor.unit ? ` ${sensor.unit}` : ""}</b><span>{formatTimestamp(sensor.points.at(-1)?.observedAt)}</span></footer></article></div> : null; })() : null}</aside></div> : null}
