...
const EXPERIENCES: Record<ExperienceKind, Experience> = {
  executive: {
    kind: "executive",
    label: { ko: "경영 보고", en: "Executive briefing" },
    nav: [
      { view: "overview", label: { ko: "브리핑", en: "Briefing" }, detail: { ko: "생산 연속성 · 경영 판단", en: "Continuity · executive decisions" } },
      { view: "reports", label: { ko: "보고서", en: "Reports" }, detail: { ko: "상황 보고 · 의사결정 기록", en: "Situation reports · decision record" } },
      { view: "objects", label: { ko: "상세 근거", en: "Evidence" }, detail: { ko: "필요할 때만 설비 단위 확인", en: "Asset detail only when needed" } },
    ],
  },
  operations: {
    kind: "operations",
...
function pageCopy(kind: ExperienceKind, view: MvpView, english: boolean) {
  if (kind === "executive") {
    ...
    if (view === "objects") return english
      ? ["EVIDENCE", "Asset-level evidence", "Use this view to validate a conclusion from the executive briefing."]
      : ["상세 근거", "설비 단위 근거 확인", "브리핑의 결론을 검증할 때 설비 단위 근거를 확인합니다."];
    return english
      ? ["BRIEFING", "Production continuity and decisions", "Prioritize business impact and decision requests over raw equipment metrics."]
      : ["브리핑", "생산 안정성과 의사결정", "설비 수치보다 경영진이 확인해야 할 영향과 판단 요청을 우선합니다."];
  }
  if (kind === "maintenance") return english
  ...
  if (kind === "engineering") return english
...
export function ReliabilityWorkspacePreview({
  ...
  const [theme, setTheme] = useState<"dark" | "light">(() => (
    window.localStorage.getItem("ontology-dashboard-theme") === "light" ? "light" : "dark"
  ));
