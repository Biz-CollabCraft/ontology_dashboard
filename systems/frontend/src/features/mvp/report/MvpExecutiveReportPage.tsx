      ...
      <article className="mvp-report-document">
        ...
        <div className="mvp-report-content-grid">
          ...
          <aside className="mvp-report-evidence-column">
            ...
            <section><span>선택 이벤트 근거</span><dl><div><dt>고장 확률</dt><dd>{formatProbability(selectedEvent.failureProbability)}</dd></div><div><dt>신뢰도</dt><dd>{CONFIDENCE_LABEL[selectedEvent.confidence]}</dd></div><div><dt>고장 유형</dt><dd>{fieldFailureLabel(selectedEvent.predictedFailureType)}</dd></div><div><dt>중요도</dt><dd>{selectedEvent.criticality ?? "확인 필요"}</dd></div><div><dt>담당자</dt><dd>{selectedEvent.assignedEngineer ?? "미배정"}</dd></div></dl></section>
            <section><span>근거 참조</span>{detail.topFactors.length ? <dl>{detail.topFactors.slice(0, 5).map((factor) => <div key={factor.id}><dt>{fieldFactorItem(factor)}</dt><dd>{factor.id}</dd></div>)}</dl> : <p>제공된 설명 요인이 없습니다.</p>}{detail.evidenceGaps.length ? <ul>{detail.evidenceGaps.map((gap) => <li key={`${gap.ownerDomain}-${gap.field}`}>{gap.field} · {gap.reason}</li>)}</ul> : null}</section>
