      ...
      <article className="mvp-report-document">
        ...
        <div className="mvp-report-content-grid">
          ...
          <aside className="mvp-report-evidence-column">
            ...
            <section><span>선택 Event 근거</span><dl><div><dt>고장 확률</dt><dd>{formatProbability(selectedEvent.failureProbability)}</dd></div><div><dt>신뢰도</dt><dd>{selectedEvent.confidence}</dd></div><div><dt>고장 유형</dt><dd>{selectedEvent.predictedFailureType}</dd></div><div><dt>중요도</dt><dd>{detail.assetCriticality ?? "확인 필요"}</dd></div><div><dt>검토 우선순위</dt><dd>{detail.reviewPriority?.level ?? "확인 필요"}</dd></div><div><dt>담당자</dt><dd>{selectedEvent.assignedEngineer ?? "미배정"}</dd></div></dl></section>
            <section><span>Evidence owner</span><p>상세 센서, Top factors, baseline/history는 Objects의 동일 snapshot을 정본으로 사용합니다. 이 보고서는 공유용 narrative와 evidence field reference만 유지합니다.</p>{detail.reviewPriority?.sourceFields.length ? <div className="mvp-report-evidence-ids"><span>Review fields</span>{detail.reviewPriority.sourceFields.map((field) => <code key={field}>{field}</code>)}</div> : null}</section>
