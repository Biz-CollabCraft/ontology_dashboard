...
import { formatProbability, formatTimestamp } from "../components/MvpUi";
...
function riskPoints(riskHistory: MvpRiskSeriesPoint[]): ChartPoint[] {
...
function sensorPoints(sensor: MvpSensorValue): ChartPoint[] {
...
function HistoryList({ detail }: { detail: MvpEventDetailModel }) {
  ...
  if (!detailMatchesSelection || !detail) {
    return (
      <div className="asset-detail-view mvp-asset-graphs" data-testid="mvp-summary-graphs">
        ...
        <div className="asset-chart-empty"><Database size={20} /><strong>AssetDetailViewModel 미연결</strong><span>Report 상세는 Backend ViewModel이 내려온 경우에만 표시합니다.</span></div>
      </div>
    );
  }
