...
export function MvpSystemAdminPage({
  ...
  return (
    <div className="mvp-system-admin-page">
      ...
      <section className="mvp-system-admin-summary" aria-label="AI 요약 처리 상태 요약">
      ...
      <section className="mvp-system-terminal" aria-label="AI 요약 처리 터미널 로그">
        <header>
          <div>
        ...
        {loading ? <MvpState kind="loading" title="AI 요약 처리 이력 조회 중" detail="저장된 AI 요약 처리 이력을 읽고 있습니다." /> : null}
        {error ? <MvpState kind="error" title="AI 요약 처리 이력 조회 실패" detail={error} onRetry={loadRuns} /> : null}
        ...
        {!loading && !error && !filteredRuns.length ? (
          <MvpState kind="empty" title="표시할 로그가 없습니다" detail="선택한 상태 필터에 해당하는 AI 요약 처리 이력이 없습니다." />
        ) : null}
      </section>
      ...
      {selectedRun ? (
        <div className="mvp-runtime-detail-layer">
          ...
          <section className="mvp-runtime-detail-dialog" role="dialog" aria-modal="true" aria-label="AI 요약 처리 상세">
            <header>
