...
function OperationalAssetsApp() {
  ...
  return <main className="ops-page">
    <header><p className="ops-eyebrow">SYSTEM OPERATIONS</p><h1>운영 자산</h1><p>Generator가 실제 사용하는 Mapping, Schema, Dataset Bundle과 Model Artifact의 검증 상태를 감독합니다.</p>{canCreateSystemAssetVersion(user?.permissions) && <button onClick={() => navigate("/system/operations/mappings/drafts")}>Mapping 새 버전 관리</button>}<button onClick={() => navigate("/system/operations/contracts")}>계약·설정 자산 관리</button><button onClick={() => navigate("/system/operations/models")}>Model 운영 선택</button><button onClick={() => navigate("/system/operations/jobs")}>Rebuild Job</button><button onClick={() => navigate("/system/operations/impact")}>Downstream 영향 분석</button><button onClick={() => navigate("/system/operations/audit")}>감사 기록</button><button onClick={() => navigate("/system/operations/logs")}>운영 로그</button><button onClick={() => navigate("/system/operations/e2e")}>E2E 실행·알림</button></header>
