...
export function ModelOperationsPage() {
  ...
  return <main className="ops-page"><button className="ops-back" onClick={() => navigate("/system/operations/assets")}>← 운영 자산</button><header><p className="ops-eyebrow">SYSTEM OPERATIONS</p><h1>Model 운영 선택</h1><p>자동 최신 버전, 운영 선택 버전과 실제 Runtime 활성 버전을 분리해 관리합니다.</p></header>
    <section className="ops-panel"><label>변경 사유<input value={reason} onChange={event => setReason(event.target.value)} /></label>{message && <p>{message}</p>}</section>
    <section className="ops-panel"><h2>모델별 버전</h2><div className="ops-table-wrap"><table><thead><tr><th>모델</th><th>latest</th><th>selected</th><th>active</th><th>상태</th><th>작업</th></tr></thead><tbody>{models.map(model => <tr key={model.model_id}><td><strong>{model.model_id}</strong></td><td>{model.latest_version ?? "—"}</td><td>{model.selected_version ?? "—"}</td><td>{model.active_version ?? "—"}</td><td>{model.selection_pending_activation ? "선택 후 미활성" : "일치"}</td><td>{canSelectSystemModels(user?.permissions) && <>{model.versions.map(version => <button key={version} onClick={() => void choose(model, version)}>{version} 선택</button>)}{model.selected_version && <button onClick={() => void clear(model)}>선택 해제</button>}</>}</td></tr>)}</tbody></table></div></section>
    ...
    <section className="ops-panel"><h2>Revision / Rollback</h2>{revisions.map(revision => <article key={String(revision.revision_id)}><strong>{String(revision.model_set_version)}</strong> · {String(revision.status)} {canRollbackSystemModels(user?.permissions) && <button onClick={() => void rollback(String(revision.revision_id))}>Rollback</button>}</article>)}</section>
  </main>;
}
