...
export function ImpactAnalysesPage() {
  ...
  const [checksum, setChecksum] = useState(""); const [jobId, setJobId] = useState(""); const [message, setMessage] = useState("");
  ...
  const create = async () => { try { const analysis = await createImpactAnalysis({ mapping_id: mappingId, mapping_version: version, mapping_sha256: checksum, rebuild_job_id: jobId, include_stages: ["preprocessing", "feature", "training"] }); navigate(`/system/operations/impact/${analysis.analysis_id}`); } catch (error) { setMessage(error instanceof Error ? error.message : "영향 분석 생성 실패"); } };
  return <main className="ops-page"><button className="ops-back" onClick={() => navigate("/system/operations/assets")}>← 운영 자산</button><header><p className="ops-eyebrow">SYSTEM OPERATIONS</p><h1>Downstream 영향 분석</h1><p>Mapping 재처리 결과가 후속 학습 파이프라인에 미치는 영향을 snapshot으로 고정합니다.</p></header>
    {canCreateImpactAnalysis(user?.permissions) && <section className="ops-panel"><h2>새 분석</h2><div className="ops-form-grid"><label>Mapping ID<input value={mappingId} onChange={e=>setMappingId(e.target.value)} /></label><label>Mapping version<input value={version} onChange={e=>setVersion(e.target.value)} /></label><label>Mapping SHA-256<input value={checksum} onChange={e=>setChecksum(e.target.value.trim())} /></label><label>완료된 Rebuild Job ID<input value={jobId} onChange={e=>setJobId(e.target.value)} /></label></div><button onClick={() => void create()}>영향 분석 생성</button>{message && <p>{message}</p>}</section>}
