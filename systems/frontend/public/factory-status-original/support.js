...
(() => {
  ...
  function parseDcDocument(doc) {
    const dc = doc.querySelector("template#factory-template");
    if (!dc) return null;
  ...
  function parseDcText(src) {
    const openMatch = /<template id="factory-template"[^>]*>/.exec(src);
  ...
  function loadReactUmd() {
    ...
    return loadScript(react.src, react.integrity).then(() => loadScript(reactDom.src, reactDom.integrity));
  }
  function init() {
