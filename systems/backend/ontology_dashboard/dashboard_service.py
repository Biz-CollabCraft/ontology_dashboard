...
class DashboardService:
    def __init__(
        ...
        *,
        ...
        self.repository = repository or DashboardRepository(database_path)

    @staticmethod
    def role_for(principal: Principal) -> str:
        if not principal.roles:
            raise AuthError(403, "role_context_denied", "Dashboard 역할이 지정되지 않았습니다.")
    ...
    def current_template(self, *, workspace_id: str, role_code: str) -> DashboardTemplateSnapshot:
        ...
        if template is None:
            raise EventNotFound(f"dashboard template {workspace_id}:{role_code}")
        return template

    def resolve(self, *, principal: Principal, workspace_id: str) -> ResolvedDashboard:
    ...
    @staticmethod
    def _dashboard_object_row(item_payload: dict[str, Any], ontology: OntologyService, workspace_id: str) -> dict[str, Any]:
        row = {
    ...
    def preview(
        ...
        *,
        ...
        if template is None:
            raise EventNotFound(f"dashboard template {workspace_id}:{role_code}:{version or 'current'}")
        preview_principal = Principal(
            user_id="template-preview",
            organization_id="template-preview",
            email="preview@ontology.local",
            display_name="Template Preview",
            status="active",
            roles=[role_code],
            permissions=[],
            workspace_scopes=[workspace_id],
            is_admin=role_code == "tenant_admin",
            default_path="/admin" if role_code == "tenant_admin" else "/app",
            landing_key=role_code,
        )
    ...
    def restore_defaults(self, *, principal: Principal, workspace_id: str) -> ResolvedDashboard:
        role_code = self.role_for(principal)
        template = self.current_template(workspace_id=workspace_id, role_code=role_code)
    ...
    def catalog(
        ...
        *,
        ...
        if role_code != principal_role and "dashboards.templates.manage" not in principal.permissions:
            raise AuthError(403, "role_context_denied", "다른 역할의 Board Catalog를 조회할 수 없습니다.")
    ...
    def list_saved_views(self, *, principal: Principal, workspace_id: str) -> list[SavedViewRecord]:
        return self.repository.list_saved_views(
            user_id=principal.user_id,
            workspace_id=workspace_id,
        )

    def get_saved_view(self, *, principal: Principal, view_id: str) -> SavedViewRecord:
        ...
        if record is None or record.workspace_id not in principal.workspace_scopes:
            raise EventNotFound(view_id)
        return record

    def delete_saved_view(self, *, principal: Principal, view_id: str) -> None:
    ...
    def publish_template(
        ...
        *,
        ...
        if current is None:
            raise EventNotFound(f"dashboard template {request.workspace_id}:{target_role}")
    ...
    def resolve_share(
        ...
        *,
        ...
        if record is None:
            raise EventNotFound("dashboard share")
