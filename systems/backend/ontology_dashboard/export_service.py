...
class ExportService:
    def __init__(
        ...
        *,
        ...
        role_workflows: RoleWorkflowService | None = None,
    ...
    def _snapshot(
        ...
        *,
        ...
        project_context = self.repository.project_context.resolve(
        ...
        if request.scope == "dashboard":
            content = self.dashboards.resolve(
                principal=principal,
            ...
            content = {
                "event": self.legacy_service.event(request.event_id),
                "evidence": evidence,
                "activity": self.legacy_service.repository.event_activity(request.event_id),
            }
        else:
            role = principal.roles[0] if principal.roles else "process_manager"
            if role == "executive_viewer" or role == "tenant_admin":
                content = self.role_workflows.executive_overview(
                    principal=principal,
                    workspace_id=request.workspace_id,
                ).model_dump(mode="json")
            elif role == "quality_auditor":
                event_id = next(iter(sorted(self.legacy_service.fixtures)))
                content = self.role_workflows.audit_reconstruction(
                    principal=principal,
                    workspace_id=request.workspace_id,
                    event_id=event_id,
                ).model_dump(mode="json")
            elif role in {"maintenance_technician", "process_engineer"}:
                content = self.role_workflows.field_workspace(
                    principal=principal,
                    workspace_id=request.workspace_id,
                ).model_dump(mode="json")
            elif role == "fde":
                content = self.role_workflows.fde_workbench(
                    principal=principal,
                    workspace_id=request.workspace_id,
                ).model_dump(mode="json")
            elif role == "ml_validator":
                content = self.role_workflows.model_console(
                    principal=principal,
                    workspace_id=request.workspace_id,
                ).model_dump(mode="json")
            else:
