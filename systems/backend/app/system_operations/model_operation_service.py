...
class ModelOperationService:
    def __init__(self, repository, generator, audit=None) -> None:
        self.repository=repository; self.generator=generator; self.audit=audit
    def _audit(self, actor, action, resource_type, resource_id, outcome="succeeded", error_code=None, version=None, reason=None):
        if self.audit: self.audit.safe_record(actor_id=actor, action=action, resource_type=resource_type, resource_id=resource_id, resource_version=version, outcome=outcome, request_id="system-model-operation", error_code=error_code, reason=reason, metadata={})
    @staticmethod
    ...
    def select(self, model_id, body, actor):
        ...
        try:
            ...
            self._audit(actor,"model.select","model_artifact",model_id,"failed","MODEL_SELECTION_FAILED",reason=body.reason)
            raise SystemOperationError(422,"MODEL_SELECTION_FAILED",str(exc)) from exc
        if result.get("idempotent"):
            self._audit(actor,"model.select","model_artifact",model_id,version=result.get("model_version"),reason=body.reason)
            return result
        ...
        self._audit(actor,"model.select","model_artifact",model_id,version=result["model_version"],reason=body.reason)
        return result
    def clear(self, model_id, body, actor):
        try: previous=self.generator.get_selection(model_id); result=self.generator.clear(model_id,{**body.model_dump(),"actor":f"system-operator:{actor}"})
            ...
            self._audit(actor,"model.selection.clear","model_artifact",model_id,"failed","MODEL_SELECTION_CLEAR_FAILED",reason=body.reason)
            raise SystemOperationError(422,"MODEL_SELECTION_CLEAR_FAILED",str(exc)) from exc
        ...
        self._audit(actor,"model.selection.clear","model_artifact",model_id,reason=body.reason)
        return result
    def validate(self, body, actor):
        try:
            result = self.generator.validate_set({**body.model_dump(),"actor":f"system-operator:{actor}"})
        except Exception as exc:
            self._audit(actor,"model_set.validate","active_model_set",body.model_set_id,"failed","ACTIVE_MODEL_SET_INVALID",body.model_set_version,body.reason)
            raise SystemOperationError(422,"ACTIVE_MODEL_SET_INVALID",str(exc)) from exc
        self._audit(actor,"model_set.validate","active_model_set",body.model_set_id,version=body.model_set_version,reason=body.reason)
        return result
    def activate(self, body, actor, action="activate"):
        ...
        try: result=operation({**request_payload,"actor":f"system-operator:{actor}"})
            ...
            self._audit(actor,f"model_set.{action}","active_model_set",body.model_set_id,"failed","ACTIVE_MODEL_SET_ACTIVATION_FAILED",body.model_set_version,body.reason)
        ...
        self.repository.record_revision(revision); self._audit(actor,f"model_set.{action}","active_model_set",body.model_set_id,version=body.model_set_version,reason=body.reason); return revision
