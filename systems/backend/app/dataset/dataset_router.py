"""FastAPI adapter for the canonical Dataset domain."""
...
def create_dataset_router(
    *,
    get_dataset_catalog_service: DependencyFactory,
    require_csrf: DependencyFactory,
    require_permission: PermissionFactory,
) -> APIRouter:
    """Build Dataset HTTP routes from composition-provided dependencies."""
    ...
    def list_datasets(
        ...
        principal: Any = Depends(require_permission("datasets.read")),
    ...
    def create_dataset(
        ...
        principal: Any = Depends(require_permission("datasets.ingest")),
    ...
    def dataset_detail(
        ...
        try:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"dataset not found: {error.args[0]}",
            ) from error
