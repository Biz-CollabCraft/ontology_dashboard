"""Deterministic SOP retrieval for read-only MVP agent packets."""
...
def retrieve_inspection_sops(
    *,
    ...
    for procedure in procedures:
        if str(procedure.get("source_kind")) not in {"demo_sop_fixture", "site_sop"}:
            continue
...
def _score_procedure(query: dict[str, Any], procedure: dict[str, Any]) -> tuple[int, list[str]]:
    ...
    required = {"asset_type", "failure_mode", "component_ids"}
