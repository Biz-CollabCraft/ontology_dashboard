...
@router.post("/demo/realtime-tick")
def create_realtime_demo_tick(
    project_id: str,
    workspace_id: str,
    dataset_version_id: str | None = Query(default=None, max_length=160),
    principal: Principal = Depends(require_permission("events.read")),
    ...
    if project_id != "manufacturing-demo-project" or workspace_id != "manufacturing-demo":
        raise HTTPException(status_code=404, detail="Realtime demo tick is only available for the manufacturing demo workspace")
    batch, tick = _demo_live_batch(
        service=service,
        principal=principal,
        project_id=project_id,
        workspace_id=workspace_id,
    ...
    if receipt.validation_status in {"accepted", "duplicate"}:
        try:
            ...
            if tick["asset_type"] == "cnc":
                observation = service.repository.save_demo_cnc_observation(
                    organization_id=principal.organization_id,
                    project_id=project_id,
                    workspace_id=workspace_id,
                    dataset_version_id=str(batch["source_context"]["dataset_version"]),
                    observed_at=datetime.fromisoformat(tick["observed_at"].replace("Z", "+00:00")),
                    asset_id=tick["asset_id"],
                    site_id=tick["site_id"],
                    cell_id=tick["cell_id"],
                    observation=tick["sensor_observation"],
                    source_sha256=str(batch["source_context"]["source_checksum"]),
                )
                tick["observation_status"] = "saved" if observation else "not_saved"
        except KeyError:
            if receipt.validation_status != "duplicate":
                raise
    if receipt.validation_status == "rejected":
        return JSONResponse(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            content={**receipt.model_dump(mode="json"), "tick": tick},
        )
    if receipt.validation_status == "conflict":
        return JSONResponse(
