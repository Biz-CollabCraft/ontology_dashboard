"""
__init__.py (RandomForestModel)

담당 기능:
- Random Forest 기반 예지보전 고장 확률 예측 모델 클래스.
...
의존 모듈:
- sklearn.ensemble.RandomForestClassifier: RandomForest 모델
- shap: TreeExplainer 기반 SHAP 수치 산출
- joblib: 모델 직렬화/복원
- systems.generator.prediction.prediction_schema.PredictionOutput: 결과 스키마

예외/경계 상황:
- 클래스 불균형(imbalanced class) 대비 class_weight="balanced" 및 n_jobs=-1 병렬 구동.
