"""
__init__.py (LightGBMModel)

담당 기능:
- LightGBM 기반 예지보전 고장 확률 예측 모델 클래스.
...
의존 모듈:
- lightgbm: LGBMClassifier 모델
- shap: TreeExplainer 기반 SHAP 수치 산출
- joblib: 모델 직렬화/복원
- systems.generator.prediction.prediction_schema.PredictionOutput: 결과 스키마

예외/경계 상황:
- SHAP 계산 시 차원에 따른 배열 형태 차이(2D vs 3D)를 안전하게 처리한다.

설계 원칙과의 연결:
- docs/architecture.md의 '독립 모델 캡슐화' 원칙에 따라 베이스 클래스 상속 없이 완전 독립적으로 구현한다.
"""
