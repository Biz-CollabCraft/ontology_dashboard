...
class ExtractionRepository:
    ...
    def get_mapping_path(self, dataset_id: str, dataset_version: str) -> Path:
        return self.mappings_dir / self._plan_filename(dataset_id, dataset_version)

    def get_mapping_uri(self, dataset_id: str, dataset_version: str) -> str:
        mapping_path = self.get_mapping_path(dataset_id, dataset_version)
        try:
            repo_root = PATHS.models_store.parent
            return str(mapping_path.relative_to(repo_root).as_posix())
        except Exception:
            return f"models_store/cache/mappings/{self._plan_filename(dataset_id, dataset_version)}"

    def find_mapping(self, dataset_id: str, dataset_version: str) -> Optional[dict[str, Any]]:
        path = self.get_mapping_path(dataset_id, dataset_version)
        if not path.exists():
            return None
        try:
            with open(path, "r", encoding="utf-8") as f:
            ...
            logger.warning(f"[ExtractionRepository] Failed to read mapping at {path}: {exc}")
            return None
    ...
    def publish_mapping(
        ...
        """Atomically stage and publish a dataset-scoped ontology mapping JSON file."""
        target_path = self.get_mapping_path(dataset_id, dataset_version)
        if target_path.exists() and not overwrite:
            logger.info(f"[ExtractionRepository] Mapping already exists at {target_path}, reusing without overwrite.")
            return self.get_mapping_uri(dataset_id, dataset_version)

        self.mappings_dir.mkdir(parents=True, exist_ok=True)
        temp_path = self.mappings_dir / f".tmp_{uuid.uuid4().hex}_{self._plan_filename(dataset_id, dataset_version)}"
        try:
            with open(temp_path, "w", encoding="utf-8") as f:
                json.dump(mapping_data, f, ensure_ascii=False, indent=2)

            temp_path.replace(target_path)
            logger.info(f"[ExtractionRepository] Atomically published ontology mapping to {target_path}")
            return self.get_mapping_uri(dataset_id, dataset_version)
        except Exception as exc:
            if temp_path.exists():
                try:
