...
- store_dir(str): 모델 저장소 경로. 기본값 "models_store".
- data_dir(str): 소스 데이터셋 폴더 경로 (get_current_snapshot 전용)
...
설계 원칙과의 연결:
- docs/architecture.md의 '독립 예측 파이프라인' 원칙에 따라 추론 및 스냅샷 조회를 독립된 서비스 모듈로 통합 제공한다.
"""
...
def run_prediction(new_rows: list[dict], store_dir: str = "models_store") -> dict:
    ...
    for name in models_entry.keys():
        model = _get_or_load_model(name, store_dir=store_dir)
        if not model:
            continue
...
def get_current_snapshot(data_dir: str = "data", n: int = 20) -> list[dict]:
    """telemetry_sensor 데이터셋 파일에서 가장 최근 n행을 스냅샷으로 조회한다."""
    sources = load_all_sources(data_dir, force_reanalyze=False)
