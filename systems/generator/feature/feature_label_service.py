...
- canonicalize_timestamp_series를 통해 시간 컬럼을 표준형(datetime64[ns])으로 정규화한 후 구간 매칭(Interval-based) 또는 정확 시각 일치 매칭을 안전하게 수행한다.
...
설계 원칙과의 연결:
- docs/architecture.md의 '시간 필드 Canonical Type 통일' 원칙에 따라 정규화된 타임스탬프로 라벨링 정확도를 보장한다.
"""
...
def build_labels(features_df: pd.DataFrame, failures_df: pd.DataFrame, failure_meta: dict | None = None) -> pd.DataFrame:
    ...
    if id_col and fail_id_col and time_col and fail_time_col and fail_time_col in f_df.columns:
        ...
        fail_events = set(zip(f_df[fail_id_col], f_df[fail_time_col]))
