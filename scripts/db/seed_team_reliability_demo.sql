...
-- It selects the highest-risk CNC RESULT# artifact and attaches a realistic
-- closed-loop path so Case KPI fields are grounded in milestone timestamps.
...
BEGIN
  ...
  WHERE organization_id = v_org
    ...
    AND artifact_id LIKE 'RESULT#%'
    AND asset_type = 'cnc'
  ORDER BY failure_probability DESC NULLS LAST, observed_at DESC, artifact_id DESC
  LIMIT 1;

  IF v_event IS NULL THEN
    RAISE EXCEPTION 'No live CNC RESULT# artifact exists for %.%. Run the current live-ingestor first.', v_project, v_workspace;
  END IF;
  ...
  v_t_started := v_observed + interval '28 minutes';
  v_t_completed := v_observed + interval '53 minutes';
  ...
  VALUES(v_maintenance_event, v_org, v_project, v_workspace, v_action, v_maintenance_wo, v_event, v_asset, v_asset, v_reco, v_decision, 'team-demo-session', 'TOOL_REPLACEMENT', jsonb_build_object('demo','presentation','risk_before','high','risk_after','reobserve'), v_t_started, v_t_completed, 'completed', v_t_completed)
  ...
  RAISE NOTICE 'Seeded Team DB Reliability demo scenario for % using %', v_asset, v_event;
END $$;
